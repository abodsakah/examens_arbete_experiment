...
export const fetchProductRecommendations = async (productId: number, size: 'small' | 'medium' | 'large' = 'medium') => {
  const response = await api.get(`/products/${productId}/recommendations?size=${size}`);
  return response.data.data;
};
